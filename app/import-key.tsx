...
export default function ImportKeyScreen() {
  ...
  const handleImportKey = async () => {
    ...
    if (!privateKeyRegex.test(trimmedKey)) {
      Alert.alert(
        'Invalid Format',
        "Private key should start with '0x' and be 66 characters long (hexadecimal).",
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        Alert.alert(
          'Invalid Key',
          'The private key provided appears invalid or corrupted.',
        );
        setIsLoading(false);
        return;
      }
      ...
      Alert.alert(
        'Wallet Imported',
        'Your wallet has been imported successfully using the private key.',
      );
      router.replace('/(tabs)');
    } catch (error) {
      // Catch potential SecureStore errors etc.
