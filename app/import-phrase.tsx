...
export default function ImportPhraseScreen() {
    ...
    const handleImportPhrase = async () => {
        ...
        if (!(words.length >= 12 && words.length % 3 === 0 && words.length <= 24)) {
             Alert.alert("Invalid Length", "Recovery phrases typically have 12, 15, 18, 21, or 24 words.");
             return;
        }
        ...
        try {
            ...
            if (!ethers.utils.isValidMnemonic(normalizedPhrase)) {
                Alert.alert("Invalid Phrase", "The recovery phrase is invalid. Please check the words and ensure they are separated by single spaces.");
                setIsLoading(false);
                return;
            }
            ...
            await SecureStore.setItemAsync('walletMnemonic_dev_unsafe', importedMnemonic);
            ...
            Alert.alert("Wallet Imported", "Your wallet has been imported successfully using the recovery phrase.");
            router.replace('/(tabs)');

        } catch (error) {
            console.error("Error importing via phrase:", error);
            Alert.alert("Import Error", "Could not import the wallet using the phrase. Please verify your input and try again.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
         <ThemedView style={styles.container}>
             ...
             <ScrollView contentContainerStyle={styles.scrollContent}>
                <Ionicons name="document-text-outline" size={56} color={Colors.dark.tint} style={styles.headerIcon} />
                <ThemedText style={styles.instructionTitle}>Import via Phrase</ThemedText>
                <ThemedText style={styles.instructionText}>
                    Enter your 12 or 24-word recovery phrase below, separated by single spaces.
                </ThemedText>
...
const styles = StyleSheet.create({
    ...
    textArea: { width: '100%', height: 120, backgroundColor: '#1C1C1E', borderRadius: 12, paddingHorizontal: 15, paddingTop: 15, fontSize: 16, color: Colors.dark.text, borderWidth: 1, borderColor: '#3A3A3C', marginBottom: 30, textAlignVertical: 'top', },
